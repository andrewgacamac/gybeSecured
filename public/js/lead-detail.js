    ...
    for (const photo of photos) {
        ...
        try {
            ...
            if (photo.original_path) {
                ...
                if (signError) {
                    console.error('DEBUG: Sign Error (Original):', signError);
                    alert('Sign Error: ' + signError.message); // ALERT ME
                } else if (data) {
                    console.log('DEBUG: Generated Signed URL:', data.signedUrl);
            ...
            if (photo.processed_path) {
                const { data, error: signError } = await supabase.storage
                    ...
                    .createSignedUrl(photo.processed_path, 3600);

                if (signError) console.error('DEBUG: Sign Error (Processed):', signError);
                else if (data) processedUrl = data.signedUrl;
            }
        } catch (e) {
            console.error('DEBUG: Exception signing URL:', e);
        }

        const div = document.createElement('div');
        div.innerHTML = `
            <div style="position: relative;">
                <img src="${originalUrl || '#'}" onclick="window.openModal('${originalUrl}')" title="Original" style="width: 100%; height: 150px; object-fit: cover; cursor: pointer;" loading="lazy" onerror="console.error('Image Load Failed:', this.src)">
            ...
            ${processedUrl ? `
                <div style="border-top: 1px solid var(--border);">
                    <img src="${processedUrl}" onclick="window.openModal('${processedUrl}')" title="Processed" loading="lazy" style="width: 100%; height: 150px; object-fit: cover; cursor: pointer;">
                </div>
            ` : ''}
...
async function approveLead() {
    ...
    try {
        ...
        const { error: dbError } = await supabase
            ...
            .update({
                status: 'APPROVED',
                approved_by: currentUser.id // Use real user ID
            })
            .eq('id', leadId);
        ...
        // Note: The function itself verifies the Authorization header (JWT)
